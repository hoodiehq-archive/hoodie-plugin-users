module.exports = function (hoodie) {
  ...
  function maybeHandleNewUser(userObject) {
    updateOldUserDoc(userObject, function(didUpdate, obj) {
      if (!didUpdate) {
        handleNewUser(obj);
      }
    });
  ...
  function updateOldUserDoc(userObject, callback) {
    if(userObject.ownerHash) {
      // we have an old user
      userObject.hoodieId = userObject.ownerHash;
      userObject.ownerHash = undefined;
      ...
      return hoodie.account.update(type, userObject.id, userObject, function(error) {
        if (error) {
          console.log('could not update legacy user doc');
          throw new Exception('could not update legacy user doc');
        }

        return callback(true, userObject);

      });
    }

    return callback(false, userObject);
  }
