...
UserAccount.prototype.changeUsername = function(currentProperties) {
  var newProperties = clone(currentProperties);
  var newUsername   = newProperties.$newUsername;
  ...
  var removeUserObject = this.removeUserObject(clone(currentProperties), newProperties)
  this.createUserObject(currentProperties, newProperties).then( removeUserObject.bind(this) );
};
...
// 
// 
UserAccount.prototype.createUserObject = function(currentProperties, newProperties) {
  this.log("creating new account '" + newProperties.name + "' for " + currentProperties.name);

  return this._promisifiedSave(newProperties._id, newProperties)
  .then( this.setProperties.bind(this) )
};
...
// 
// 
UserAccount.prototype.removeUserObject = function(oldProperties, newProperties) {
  return function() {
    this.log("removing " + newProperties.name + "'s old account: " + oldProperties.name)
    var remove = this.promisify(this.worker.usersDatabase, 'remove')
    return remove(oldProperties._id, oldProperties._rev)
  }.bind(this)
};
