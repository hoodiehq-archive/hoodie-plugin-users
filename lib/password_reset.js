...
var PasswordReset = function(properties, worker) {
  ...
  .fail( this.markDocAsFailed.bind(this) );
};
...
PasswordReset.prototype.checkIfUserExists = function() {
  ...
  return get(this.userId).fail( this.handleUserNotFound.bind(this) );
};
...
PasswordReset.prototype.handleUserNotFound = function(error) {
  return this.worker.rejectWith("An account with the email address " + this.emailAddress + " could not be found");
};
...
PasswordReset.prototype.sendNewPassword = function() {
  ...
  return send(email).fail( this.handleSendEmailError.bind(this) );
};
...
PasswordReset.prototype.cleanupPasswordReset = function(doc) {
  this.log('cleaning up ' + this.properties._id);
  return this.worker.promisify( this.worker.usersDatabase, 'remove')(this.properties._id, this.properties._rev)
  .then( function() {
    this.emit('deleted');
    this.smtpTransport.close();
  }.bind(this));
...
PasswordReset.prototype.markDocAsFailed = function(error) {
  ...
  this.userDb.save(this.properties._id, this.properties._rev, this.properties, function (error, res) {
    if (error) {
