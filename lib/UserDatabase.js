...
//
UserDatabase.prototype.drop = function(name) {

  this.log('dropping database %s ...', name);
  ...
  setTimeout( function() {
    this.couch.database(name).destroy( function(error) {
      if(error !== null) {
        ...
        this.log("error deleting user db");
        this.log( JSON.stringify(error, "", 2) );
        
        return;
      }
      
      this.log("database killed");
    }.bind(this));
  }.bind(this), 3000)
};
